...
const LOGO_LIST_IM = {
	...
	"IM_BLACK_ON_WHITE": new OperationPeriod(new Collection(1990, Season.S), null,
		col => col.isBeforeOrIn(new Collection(2000, Season.W))
				|| col.isAfterOrIn(new Collection(2002, Season.W))),
	"IM_WHITE_ON_BLACK": new OperationPeriod(new Collection(2000, Season.W), new Collection(2002, Season.W)),
};
const LOGO_LIST_ME = {
	"ROUND": new OperationPeriod(null, new Collection(1983, Season.W)),
	"BRUSH": new OperationPeriod(new Collection(1983, Season.S), new Collection(1990, Season.S)),
	"IM_WHITE_ON_BLACK": new OperationPeriod(new Collection(1990, Season.S), new Collection(2011, Season.W),
		col => !col.isBetween(new Collection(1995, Season.W), new Collection(2002, Season.S))),
	"ME_BLACK_ON_WHITE": new OperationPeriod(new Collection(1995, Season.W), new Collection(2002, Season.S)),
	"ME_WHITE_ON_BLACK": new OperationPeriod(new Collection(1998, Season.W), new Collection(2020, Season.W),
		col => !col.isBetween(new Collection(2002, Season.W), new Collection(2011, Season.W))),
};
const LOGO_LIST_CL = {
	"IM_WHITE_ON_BLACK": new OperationPeriod(new Collection(1994, Season.S), new Collection(1994, Season.W))
}
...
const LINE = deepFreeze({
	...
	CL: new Line("COTTON LABEL ISSEY MIYAKE",
			new OperationPeriod(new Collection(1994, Season.S), new Collection(1996, Season.W)),
			LOGO_LIST_CL),
