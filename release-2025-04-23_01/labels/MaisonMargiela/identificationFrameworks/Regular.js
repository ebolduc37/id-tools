...
const LABEL_NOCODE = ['20', 'H_M', 'SUP', 'UNSPECIFIED'];
const MANUFACTURER_NOCODE = ['MACKINTOSH'];
const LINENAME_ARTISANAL = [LINE['(0)'].name, LINE['(0)(10)'].name];
const COL_NOCODE_LAST = new Collection(1998, Season.S);
const COL_MISS_DEANNA_LAST = new Collection(2003, Season.W);

const LINE_PRE = {
    '(1)': new OperationPeriod(new Collection(2010, Season.P), new Collection(2022, Season.R), // Resort 2015 last
            col => !col.isBetween(new Collection(2015, Season.S), new Collection(2021, Season.W))), // Resort 2022
    'MM6': new OperationPeriod(new Collection(2015, Season.R))
};
...
InputMM.prototype.isRegular = function() {
    ...
    if (this.isLabelNotationArtisanal() && !this.isLabelNotationBLANK()) {
        ...
        if (!(this.isManufacturerMADE_IN_FRANCE() || this.isManufacturerUnspecified())) return false;
    }
    if (this.isManufacturerMADE_IN_FRANCE()) {
        if (this.isProductCodeString()) return false;
...
InputMM.prototype.extract_Regular = function() {
    ...
    if (this.isProductCodeString() && !nocode_exception) {
        ...
        if (seasonID === '1') {
            preCollection = new Collection(year, Season.R);
            collection = new Collection(year, Season.S);
        }
        else if (seasonID === '2') {
            preCollection = new Collection(year, Season.P);
            collection = new Collection(year, Season.W);
        }
    }
    ...
    for (let line of LABEL_NOTATION[labelNotation].lineList) {
        ...
        if (LINENAME_ARTISANAL.includes(line.name)) {
            ...
            else if (!(this.isManufacturerMADE_IN_FRANCE() || this.isManufacturerUnspecified())) continue;
        }
        else if (this.isManufacturerMADE_IN_FRANCE()) continue;

        // Editing the line's name if crafted by Mackintosh
        if (this.isManufacturerMACKINTOSH()){
            line = line.copy();
        ...
        // Initialize pre-collection parameters
        let isPreCollection = false;
        let operationPeriod_pre = new OperationPeriod();
        for (const [key_line, value_OP] of Object.entries(LINE_PRE)) {
            if (line.name === LINE[key_line].name) {
                isPreCollection = true;
                operationPeriod_pre = value_OP;
            }
        }

        // Limiting the set of candidates to the collections with a season
        candidates = candidates.filter(col => col.isCollection()
            || (isPreCollection && col.isPreCollection() && operationPeriod_pre.includes(col)));
        ...
        // Limiting the set of candidates when the product code is blank
        if (this.isProductCodeBlank() && !LINENAME_ARTISANAL.includes(line.name)) {
            if (this.isManufacturerMISS_DEANNA() || this.isManufacturerUnspecified())
                candidates = candidates.filter(col => col.isBeforeOrIn(COL_MISS_DEANNA_LAST));
            ...
            candidates = candidates.filter(col => col.isEqualTo(collection)
                || (isPreCollection && col.isEqualTo(preCollection)));
        }
