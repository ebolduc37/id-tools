...
import { ALL_COLLECTIONS, IM_INC_ACCESSORY_TYPE_ID as ACCESSORY_TYPE_ID,
    IM_INC_COUNTERFEITS as COUNTERFEITS, IM_INC_FABRIC_TYPE_ID as FABRIC_TYPE_ID,
    IM_INC_GARMENT_TYPE_ID as GARMENT_TYPE_ID, IM_INC_LINE_ID as LINE_ID,
    IM_INC_SEASON_ID as SEASON_ID, 
    LINE} from "../constants/index.js";
const Season = Collection.Season;
const REGEX_IM_INC = /^[A-Z]{2}\d{2}[A-Z]{2}\d{3}\d?\d?$/;
const FRAMEWORK_IM_INC = "ISSEY MIYAKE INC.";
const MANUFACTURER_IM_INC = "ISSEY MIYAKE INC. [Japanese: 株式会社 イッセイ ミヤケ]";

// Common alphabetical sizing ending
...
// Common numerical sizing beginning
...
const COL_TYPEFACE_SANS_PROTO_END = new Collection(1994, Season.S);

// Slab serif type ending and sans serif beginning
const COL_TYPEFACE_SLAB_END = new Collection(2000, Season.W);
const COL_TYPEFACE_SANS_START = new Collection(2000, Season.S);
 ...
 */
InputIM.prototype.isIM_INC = function() {
...
InputIM.prototype.extract_IM_INC = function() {
    ...
    let identification = new Identification({
        ...
        framework: FRAMEWORK_IM_INC,
        manufacturer: MANUFACTURER_IM_INC,
        input: this.copy()
    });
    ...
    for (let line of LINE_ID[lineID]) {
        ...
        else if (productCode.slice(0,3) === "PP0" && name === LINE.IM.name && this.isTypefaceSlabSerif())
        ...
        if (this.isTypefaceSlabSerif())
            ...
            candidates = candidates.filter(col => col.isBeforeOrIn(COL_TYPEFACE_SANS_PROTO_END)
                                                || col.isAfterOrIn(COL_TYPEFACE_SANS_START));
