...
// Constants
import { ALL_COLLECTIONS, COUNTERFEITS, FROM_MONTHLY_TO_SEASONAL, GARMENT_TYPE_ID, LINE_ID, LINE, SEASON_ID, TITLES } from "../constants/index.js";
...
const SEASON_ID_EXTRAS = {
    C: {
        name: LINE.PPOH.name,
        extraCol: new Collection(2019, Season.W)
    },
    P: {
        name: LINE.HP.name,
        extraCol: new Collection(2006, Season.S),
        extraText: "[only for collaborative pieces with the Rolling Stones]"
    }
};
const firstSeasonalCol = new Collection(2001, Season.W);
...
InputCDG.prototype.extract_Seasonal = function() {
    ...
    for (let line of LINE_ID[lineID]) {
        ...
        // Limiting the set of candidates to the collections occuring after the change to the seasonal code
        candidates = candidates.filter(col => col.isAfterOrIn(firstSeasonalCol));
    ...
    COUNTERFEITS.forEach((counterfeit) => {
        if (this.isInputCDGEqualTo(counterfeit))
