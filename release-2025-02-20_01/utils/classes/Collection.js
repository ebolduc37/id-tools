...
class Collection {
     ...
     */
    isHalfYearBefore(col) {
        if (col == null) return false;
        return (this.isR() && col.isP() && col.year === this.year)
            || (this.isS() && col.isW() && col.year === this.year)
            || (this.isP() && col.isR() && col.year === this.year + 1)
            || (this.isW() && col.isS() && col.year === this.year + 1);
    }
    ...
    isBeforeOrIn(col) {
        ...
        else return this.isR() || (this.isS() && !col.isR()) || (this.isP() && col.isP()) || col.isW();
    };
