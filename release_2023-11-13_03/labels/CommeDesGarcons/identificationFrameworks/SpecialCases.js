...
InputCDG.prototype.isSpecialCase = function() {
    ...
    if (productCode in SPECIAL_CASES)
        return this.isYearPrintEqualTo(SPECIAL_CASES[productCode].yearPrint);
 ...
 */
InputCDG.prototype.identifySpecialCases = function() {

    // Returning null if the input data is not a valid COMME des GARÇONS special case
    if (!this.isSpecialCase()) return null;

    // Initializing useful parameters
    const productCode = this.getProductCodeStandardized();

    // Initializing the identification object
    let identification = SPECIAL_CASES[productCode].identification.copy();
