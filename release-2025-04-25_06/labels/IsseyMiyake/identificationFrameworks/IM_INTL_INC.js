...
// Constants
import { ALL_COLLECTIONS, IM_INTL_INC_COUNTERFEITS as COUNTERFEITS, 
    IM_INTL_INC_FABRIC_TYPE_ID as FABRIC_TYPE_ID,
    IM_INTL_INC_GARMENT_TYPE_ID as GARMENT_TYPE_ID, IM_INTL_INC_LINE_ID as LINE_ID,
    IM_INTL_INC_SEASON_ID as SEASON_ID } from "../constants/index.js";
const Season = Collection.Season;
const REGEX_IM_INTL_INC = /^[A-Z]{2}\d{5}$/;
const FRAMEWORK_IM_INTL_INC = "ISSEY MIYAKE INT'L. INC.";
const FRAMEWORK_ON_LIMITS = "ISSEY MIYAKE ON LIMITS INC. as INT'L. INC.";
const MANUFACTURER_IM_INTL_INC = "ISSEY MIYAKE INT'L. INC. [Japanese: ㈱ イッセイ ミヤケ インターナショナル]";
 ...
 */
InputIM.prototype.isIM_INTL_INC = function() {
    ...
    //if (!this.isManufacturerIM_INTL_INC() && !this.isManufacturerUnspecified()) return false;
    if (!(this.isManufacturerIM_INTL_INC() || this.isManufacturerON_LIMITS() || this.isManufacturerIM_INC())
    ...
    if (!REGEX_IM_INTL_INC.test(productCode)) return false;
...
InputIM.prototype.extract_IM_INTL_INC = function() {
    ...
    let identification = new Identification({
        ...
        framework: FRAMEWORK_IM_INTL_INC,
        manufacturer: MANUFACTURER_IM_INTL_INC,
        input: this.copy()
    });
    ...
    else if (this.isManufacturerIM_INC())
    ...
    //if (this.isTypefaceSansSerif()) stylizedCode = productCode.slice(0, 4) + "-" + productCode.slice(4);
