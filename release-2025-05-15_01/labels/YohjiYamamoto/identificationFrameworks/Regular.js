...
const COL_DONOTTUMBLEDRY_KANJI_START = {
...
InputYY.prototype.extract_Regular = function() {
    ...
    for (let line of LINE_ID[lineID]) {
        ...
        if (this.isDoNotTumbleDryShort() || this.isDoNotTumbleDryHIRAGANA()) {
        ...
        else if (this.isDoNotTumbleDryKANJI()) {
            let col_limit = COL_DONOTTUMBLEDRY_KANJI_START["DIFF"];
            if (line.name === LINE.YS.name)
                col_limit = COL_DONOTTUMBLEDRY_KANJI_START["YS"];
            else if (LINE_MAIN.includes(line.name))
                col_limit = COL_DONOTTUMBLEDRY_KANJI_START["MAIN"];
            candidates = candidates.filter(col => col.isAfterOrIn(col_limit));
        }
