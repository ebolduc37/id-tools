 ...
 */
InputYY.prototype.idList_Regular = function() {
    ...
    let selection = Object.values(LINE);
    if (lineID != null)
    ...
    for (let line of selection) {
        ...
        // Jumping the loop if the logo list of the line is empty
        if (line.isLogoListEmpty()) continue;

        // Limiting the set of candidates to the collections with the right logo style if entered
        if (this.isLogoEntered()) {
