 ...
 * @enum {string}
 */
const NoProductCodeType = Object.freeze({
    BLANK: "BLANK",
...
class Input {
    ...
    static Label = Label;
    static NoProductCodeType = NoProductCodeType;

    /**
     * Create an object containing the input data.
     * @param {Input.Label} [label] - Object to extract the label from.
     * @param {(string|Input.NoProductCodeType)} [productCode] - Object to extract the product code from.
     * @param {string} logoStyle - Object to extract the logo style from.
     */
    constructor({ label, productCode = Input.NoProductCodeType.BLANK, logoStyle = null }) {
        /** @type {Input.Label} */
        this.label = label;
        /** @type {(string|Input.PRODUCT_CODE_PRINT_TYPES)} */
        this.productCode = productCode;
        if (!standardize(productCode)) this.productCode = Input.NoProductCodeType.BLANK;
        /** @type {string} */
    ...
    isProductCodeString() {
        return typeof this.productCode === "string"
                && !Object.values(Input.NoProductCodeType).includes(this.productCode);
    }
     ...
     */
    isProductCodeBlank() {
        return this.productCode === Input.NoProductCodeType.BLANK;
    }

    /**
     * Evaluate if the product code print is indefinite.
     * @return {boolean} True if it is; false otherwise.
     */
    isProductCodeIndefinite() {
        return this.productCode === Input.NoProductCodeType.INDEFINITE;
    }
    ...
    isProductCodeUnknown() {
        return this.productCode === Input.NoProductCodeType.UNKNOWN;
    }
     ...
     * @param {(string|Input.NoProductCodeType)} productCode - Product code to compare with.
    ...
    isProductCodeEqualTo(productCode) {
        ...
        if (this.isProductCodeIndefinite() || input.isProductCodeIndefinite()) return true;
    ...
    confirmProductCode() {
        ...
        else if (this.isProductCodeIndefinite()) message = "indefinite";
    ...
    identify() {
        ...
        if (this.isValid()) results = this.idList();
