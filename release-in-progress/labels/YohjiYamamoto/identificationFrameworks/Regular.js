...
const LOGO_STYLES = InputYY.LOGO_STYLES;
...
InputYY.prototype.identifyRegular = function() {
    ...
    for (let line of selection) {
        ...
        // Limiting the set of candidates to the collections with the right logo style
        if (Object.keys(line.logoList).length === 0) continue;
        else {
            let match = false;
            for (const [key, value] of Object.entries(line.logoList)) {
                if (LOGO_STYLES[key] === this.logoStyle || key == "ANY") {
                    candidates = candidates.filter(col => value.includes(col));
                    match = true;
                }
            }
            if (!match) continue;
        ...
        if (this.isSizingAlphabetical() && !SIZING_ALPHABETICAL_EXCEPTIONS.includes(line.name))
